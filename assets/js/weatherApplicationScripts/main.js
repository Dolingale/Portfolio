...
function APICall(long, lat) {
    ...
    .then((data) => {
        ...
        if (thisHour >= 6 && thisHour < 18) {
            logo.src = `data/weatherApplicationRessources/jour/${APIresults.current.weather[0].icon}.svg`;
        } else{
            logo.src = `data/weatherApplicationRessources/nuit/${APIresults.current.weather[0].icon}.svg`;
        }
